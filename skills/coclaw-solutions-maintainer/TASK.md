...
2. 再读 `SKILL.md`（能力与强约束：脚本职责拆分；评论发布采用逐条 `gh issue comment` 立即发布）
3. 再读 `USAGE.md`（Runbook：具体命令怎么跑、输出路径在哪）
4. 在撰写/发布任何 GitHub issue comment 之前，必须阅读 KB 索引入口：`docs/kb/INDEX.md`（若不存在或内容过期，先执行 `pnpm kb:build` 生成/刷新）
...
2. 在回复末尾**推荐 1–2 个 `coclaw.com` 相关页面链接（默认需要）**，用于延伸阅读/更完整步骤：
   ...
   - 不要求必须是 solution 链接
   - **链接格式强制使用 Markdown：`[title](url)`（不要裸 URL）**
...
5. 回帖前阅读 KB 索引（避免凭记忆挑链接/猜配置）：
   - 入口：`docs/kb/INDEX.md`
   - 站内页面索引：`docs/kb/coclaw-site-pages.md`（用于挑选 1–2 个推荐链接）
...
8. **立即发布评论（逐条，不要批量等到最后）**：
   ...
   - 若 `gh` 未登录/无权限导致无法发布：停止继续处理后续 issues，在最终回复中说明失败原因与恢复步骤
9. 若需要新增/更新站内内容：
   - 先写/改内容并 `pnpm build` 通过
   - **commit 并 push 到 `main`**
   - 再发布 GitHub 回复并附上新页面链接（避免用户点开不可用）
   - 注意：此分支下的回帖仍遵循“每条准备好就立刻发布”，不要攒到最后一起发
...
- **发布策略：逐条立即 `gh issue comment`**（每完成一条就提交，不等所有 issues 都写完，也不需要用户审核）
...
- 在本轮识别出的“使用问题”队列中，选择优先级最高/信息最完整的 issues，**最多评论 10 条**，且每条符合质量标准（默认包含站内推荐链接），并且每条已实际执行 `gh issue comment` 发布成功。
...
每轮任务结束后，在对话中输出一份“本轮汇总报告”，用于让维护者快速复盘与追踪后续工作。要求：

- 结构固定如下（可按需省略“无内容”的小节，但顺序不变）。
- 所有链接**强制使用** `[title](url)`（不要裸 URL）。
- 内容要“可审计”：包含数量、结论、失败原因与下一步；避免粘贴大段日志。

模板（直接按此结构填充）：

### 标题

`# CoClaw Solutions Maintainer — 本轮汇总（YYYY-MM-DD）`

### 概览（Overview）

- 时间范围：`<since>` → `<until>`（例如最近 72h）
- 本轮上限：最多评论 10 条；站内内容变更 0–3 篇（可选）
- 数据与产物：
  - triage：`<path/to/triage-latest.json>`
  - classification：`<path/to/classification-latest.json>`
- 统计：
  - 候选 issues：`<N>`
  - 已发布评论：`<N>`
  - 跳过/延后：`<N>`
...
### 已发布评论（Published Comments）

用表格列出（每行 1 个 issue）：

| Issue                                                            | 类型           | 结论（1 行）          | 推荐站内链接（1–2） |
| ---------------------------------------------------------------- | -------------- | --------------------- | ------------------- |
| `[Issue Title](https://github.com/openclaw/openclaw/issues/123)` | `usage_config` | `<做了什么 + 为什么>` | `[title](url)`      |

说明：

- “结论”只写关键步骤/关键判断，不重复 issue 原文。
- 若在评论中请求了补充信息，结论中要点明“缺什么”。

### 跳过/延后（Skipped / Deferred）

用表格列出（每行 1 个 issue）：

| Issue                                                            | 原因                                                   | 需要的下一步                            |
| ---------------------------------------------------------------- | ------------------------------------------------------ | --------------------------------------- |
| `[Issue Title](https://github.com/openclaw/openclaw/issues/456)` | `code_bug / feature_request / 信息不足 / 超出本轮上限` | `<跳过/要等什么/要问什么/下一轮再处理>` |

### 站内内容变更（Site Content Changes, if any）

- 新增/更新页面：
  - `[title](url)`：`<一句话说明改动价值>`
- 构建与发布：
  - `pnpm build`：`pass/fail`
  - Git：`<commit/PR/branch>`（如适用）

### 风险与后续（Risks & Follow-ups）
