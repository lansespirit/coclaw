# 任务 11：撰写《ClawHub 使用指南》

## 执行状态

- 状态：已完成
- 完成日期：2026-03-08
- 输出位置：`src/content/guides/clawhub-usage-guide.mdx`
- 输出集合：`guides`

## 任务使命

聚焦 ClawHub 作为技能/扩展发现与分发入口的真实价值与风险，写一篇面向实操的指南。

## 目标读者

准备安装社区技能，但不清楚怎样评估与回滚的用户。
...
- `src/content/stories/haoyang-zhang-evomap.mdx`
- `src/content/troubleshooting/solutions/clawhub-cli-unauthorized-with-valid-token.mdx`
...
- 解释 ClawHub 在整个生态中的角色。
- 告诉用户如何发现、评估、安装、升级、回滚一个 skill。
- 强调供应链与权限风险。
- 给出判断一项社区 skill 是否值得装的检查清单。
- 不要把文章写成单纯“去哪里点按钮”的界面教程。
