# 任务 02：撰写《把 OpenClaw 接进 Home Assistant》

## 执行状态

- 状态：已完成
- 完成日期：2026-03-08
- 输出位置：`src/content/guides/home-assistant-openclaw-integration.mdx`
- 输出集合：`guides`

## 任务使命

围绕 Home Assistant 社区对 OpenClaw 的兴趣，写一篇偏实战的 how-to / architecture guide，解释把 OpenClaw 接入家庭自动化系统的现实路径、能力边界与注意事项。

## 目标读者

已经有 Home Assistant 或正在搭家庭自动化系统，希望把 OpenClaw 当成家庭语音/自动化中枢的用户。
...
- 明确 Home Assistant + OpenClaw 的典型价值：语音入口、自动化决策、家庭状态问答、通知聚合。
- 说明最现实的集成模式，而不是追求全自动炫技。
- 讨论安全边界：家庭设备控制权限、确认机制、误触发、访客语音输入。
- 讨论适合用规则、适合用 agent 的边界。
- 给出“最小可行集成”的分层路线图。
...
一篇 2200-3500 字的 guide，适合收录到 `guides`。
