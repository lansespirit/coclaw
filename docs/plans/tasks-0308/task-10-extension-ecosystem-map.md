# 任务 10：撰写《OpenClaw 扩展生态地图》

## 执行状态

- 状态：已完成
- 完成日期：2026-03-08
- 输出位置：`src/content/blog/openclaw-extension-ecosystem-map.mdx`
- 输出集合：`blog`

## 任务使命

围绕 skills、ClawHub、ACPX、社区插件与周边仓库，写一篇真正意义上的生态地图。

## 目标读者

知道 OpenClaw 不只是主仓库，但不清楚生态各模块之间关系的用户。
...
- 解释各生态组件的角色，而不是简单列仓库名单。
- 说明普通用户什么时候会接触到这些生态组件。
- 加入成熟度、风险、适用场景、维护信号等判断维度。
- 把“装扩展”从功能追求拉回到运营与安全权衡。
- 避免只做简介页，要有选型价值。
