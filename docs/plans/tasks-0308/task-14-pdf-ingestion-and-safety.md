# 任务 14：撰写《让 OpenClaw 读 PDF，但不要让它被 PDF 带偏》

## 执行状态

- 状态：已完成
- 完成日期：2026-03-08
- 输出位置：`src/content/blog/openclaw-pdf-ingestion-safety.mdx`
- 输出集合：`blog`

## 任务使命

围绕 PDF 总结、文件摄取和 prompt injection 风险，写一篇兼顾需求与安全边界的文章。

## 目标读者

想把 OpenClaw 用于读资料、总结 PDF、处理附件的用户。
...
- `src/content/guides/openclaw-email-gmail-setup-and-reliability.mdx`

## 必须核对的外部讨论与信号

- https://github.com/openclaw/openclaw/issues/39543

## 必须回答的问题

- 解释为什么 PDF 总结是高频真实需求，而不是边缘功能。
- 讨论文档摄取与 prompt injection 的关系。
- 区分“读资料”“抽取结构化信息”“执行基于文档的动作”这几层风险。
- 给出较稳妥的使用模式建议。
- 不要写成一个空泛的安全警示贴。
