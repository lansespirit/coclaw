# 任务 08：撰写《OpenClaw 稳定性手册》

## 执行状态

- 状态：已完成
- 完成日期：2026-03-08
- 输出位置：`src/content/blog/openclaw-stability-playbook.mdx`
- 输出集合：`blog`

## 任务使命

将 GitHub 上 no output、false rate limit、重复发消息、上下文错乱等问题，上升为“运行稳定性”视角的一篇总览手册。

## 目标读者

已经把 OpenClaw 跑起来，但正在被不稳定行为困扰的用户。
...
- `src/content/guides/openclaw-operability-and-observability.mdx`
- `src/content/guides/openclaw-deployment-troubleshooting.mdx`

## 必须核对的外部讨论与信号

- https://github.com/openclaw/openclaw/issues/5030
- https://github.com/openclaw/openclaw/issues/32828
- https://github.com/openclaw/openclaw/issues/39536
- https://github.com/openclaw/openclaw/issues/39528

## 必须回答的问题

- 从“症状”视角归类，而不是逐个 issue 讲故事。
- 总结常见失控模式：没输出、假限流、重复发送、上下文丢失、未授权动作。
- 给出稳定性设计原则：超时、熔断、重试、告警、观察面、人工确认。
- 让读者知道上线前最该检查哪些点。
- 避免变成单篇排障合集，要有体系。
