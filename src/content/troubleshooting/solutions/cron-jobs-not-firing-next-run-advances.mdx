...
seoTitle: "Fix Cron: jobs don't fire and nextRunAtMs silently advances"
seoDescription: "Fix cron jobs that skip runs (no execution/logs) while nextRunAtMs advances by upgrading past known scheduler regressions and verifying cron storage + timezone."
reviewedBy:
...
- Variant: cron jobs fire on gateway start (startup catch-up), then **stop firing entirely**
...
If you are hitting the “runs only at startup catch-up” variant, treat it as a scheduler
stall until proven otherwise.
...
- OpenClaw CLI: [`openclaw cron`](https://docs.openclaw.ai/cli/cron)
- GitHub: #12502, #43937
