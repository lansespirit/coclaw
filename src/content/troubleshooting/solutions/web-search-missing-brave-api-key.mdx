...
seoTitle: "Fix Web search: missing_brave_api_key even though it's confi"
seoDescription: "If `web_search` fails with `missing_brave_api_key` but you set the key via `openclaw configure --section web`, make sure the running gateway service can see..."
reviewedBy:
